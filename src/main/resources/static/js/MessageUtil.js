var Msg = {
  ...
  showMsg: function (msgTitle, showtime, htmlTager) {
    ...
    myIframe.className = 'myIframe';
    myIframe.style.top = "50%";
    myIframe.style.display = "block";
    //myIframe.style.color = "red";
    myIframe.style.color = "red";
    ...
    myIframe.style.width = document.getElementById("MessageBox").offsetWidth + "px";//设置iframe与弹出层宽度相同
  ...
  tbsuccess: function (msgTitle, showtime) {
    if (msgTitle == null) {
      msgTitle = "提交成功";
    }
    if (showtime == null) {
  ...
  wait: function (msgTitle, showtime) {
    ...
    Msg.showDisplay();
    var tager = "<table cellpadding='0px' cellspacing='0px'><tr><td class='wait_left'><img src='" + root + "/style/images/message/loading.gif' style='vertical-align:middle;'></img></td><td class='msg_content'>" + msgTitle + "</td><td class='msg_right'></td></tr></table>";
    Msg.showMsg(msgTitle, showtime, tager);
    if (showtime != null) {
      setTimeout(Msg.hide, showtime * 1000);
    }
  },
  ...
  removeAlertItem: function () {
    ...
    if (md != null) {
      ...
      document.body.removeChild(myIframe); //删除提示框
    }
  },
